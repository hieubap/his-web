...
const InfoRegister = (props) => {
  const { getDetail, id, updateData, hangThe, hangTheId, } = props;
          ...
          {hangThe && hangThe?.icon && (
            <div className="hangTheIcon">
              ...
              <PopoverWrapper
                ...
                trigger="hover"
              >
                <img src={`${fileUtils.absoluteFileUrl(hangThe?.icon)}`} alt="" />
              </PopoverWrapper>
            </div>
          )}
          <Image
...
const mapStateToProps = (state) => {
  return {
    ...
    hangTheId: state.tiepDon.hangTheId || "",
  };
};
...
export default compose(withConnect, memo)(InfoRegister);
