...
const WebcamModal = ({ image, modalActions, title, show, onClose, ...props }) => {
  ...
  const selectImage = (data, isCapture, upload) => {
    let type = data.target && data.target.files && data.target.files[0] && data.target.files[0].type
    if (type === "image/png" || type === "image/jpeg" || type === "image/gif" || upload) {
      let file = "";
      let fileUpload = "";
      let urlPreview = "";
      let fileName = "";
        ...
        <div style={{ display: "flex", padding: "5px", justifyContent: "space-between", }} >
          <div className={"selectImage"}>
            <input
              style={{ display: "none" }}
              accept="image/*"
              type="file" onChange={(e) => selectImage(e)}
              ref={refImage}
            />
          ...
          <div className="action">
            <Button type="danger" className="btn-create" onClick={reTakePhoto} size="large">
              Chụp lại
            </Button>
            <Button type="primary" className="btn-create" htmlType="submit" size="large" onClick={uploadImage}>
              Upload
            </Button>
          </div>
        </div>}
        ...
        <div className="image-preview">
          {state.urlPreview ?
            ...
            : <div className="camera">
              <Camera onTakePhoto={dataUri => { selectImage(dataUri, true, true); }} isImageMirror={true} />
            </div>}
        </div>
      </Spin>
    </Main>
