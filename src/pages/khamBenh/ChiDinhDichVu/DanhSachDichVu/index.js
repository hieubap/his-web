...
import { Checkbox, Form, Input, Row, Col, message ,Select} from "antd";
...
const DanhSachDichVu = (props) => {
  ...
  const nbDotDieuTriId = useSelector((state) => state.khamBenh.infoNb?.nbDotDieuTriId);
  ...
  const handleVisible = (key, data) => (e) => {
    ...
    searchPhongThucHien({ soPhieuId: dataPhieuChung.soPhieuId, nbDotDieuTriId: nbDotDieuTriId });
  ...
  const handleSubmit = (data) => () => {
    form.validateFields().then(async (values) => {
      ...
      themThongTinPhieu({ body: obj, id: soPhieuId, loaiDichVu }).then(
        (s) => {
          if (s.code === 0) {
            setVisible([]);
            form.resetFields();
