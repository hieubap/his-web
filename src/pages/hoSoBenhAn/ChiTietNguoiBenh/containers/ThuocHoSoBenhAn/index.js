...
const ThuocHoSoBenhAn = ({
  ...
  useEffect(() => {
    setState({
      ...state,
      data: listDvThuoc.filter((item) => item.maHoSo === selectedMaHs),
      dataRender: listDvThuoc.filter((item) => item.maHoSo === selectedMaHs),
    });
  }, [listDvThuoc, selectedMaHs]);
    ...
    },
    {
      title: (
        <HeaderSearch
          ...
          onClickSort={handleSort(true)}
          dataSort={
            (state.sort?.key === "tenLieuDung" && state.sort?.value) || 0
          }
        />
      ),
