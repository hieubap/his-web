...
const LoiDan = (props) => {
  ...
  useEffect(() => {
    ...
    getListLoiDan(params);
  }, []);

  useEffect(() => {
    const data = listLoiDan.map((item, index) => {
      return { ...item, action: item, stt: page * size + index + 1 };
    });
    setData(data);
  }, [listLoiDan, page, size]);
    ...
    },
    {
      title: (
        <HeaderSearch
          ...
          sort_key="danhChoThuoc"
          onClickSort={onClickSort}
          dataSort={dataSortColumn.danhChoThuoc || 0}
          searchSelect={
            <Select
              onChange={(e) => {
                onSearchInput(e, "danhChoThuoc");
              }}
              defaultValue=""
              placeholder={"Chọn danh cho thuốc"}
              data={DANH_CHO_THUOC}
            />
          }
        />
      ...
      <HomeWrapper title="Danh mục">
        ...
        >
          <TableWrapper
            ...
            buttonHeader={
              checkRole([ROLES["DANH_MUC"].LOI_DAN_THEM])
                ? [
                    {
                      title: "Thêm mới [F1]",
        ...
        {!state.showFullTable && (
          ...
          >
            <CreatedWrapper
              ...
              onOk={handleAdded}
              okText="Lưu [F4]"
              roleSave={[ROLES["DANH_MUC"].LOI_DAN_THEM]}
              roleEdit={[ROLES["DANH_MUC"].LOI_DAN_SUA]}
              editStatus={editStatus}
            >
              <Form
                disabled={
                  editStatus
                    ? !checkRole([ROLES["DANH_MUC"].LOI_DAN_SUA])
                    : !checkRole([ROLES["DANH_MUC"].LOI_DAN_THEM])
                }
                form={form}
                layout="vertical"
                ...
                <Form.Item
                  ...
                  rules={[
                    ...
                    },
                    {
                      max: 1000,
                      message:
                        "Vui lòng nhập tên lời dặn không quá 1000 ký tự!",
                    },
                    {
                      whitespace: true,
                      message: "Vui lòng nhập tên lời dặn!",
...
const mapDispatchToProps = ({
  loiDan: { getListLoiDan, createOrEdit, onDelete, updateData },
}) => ({
  getListLoiDan,
  createOrEdit,
  onDelete,
...
export default connect(mapStateToProps, mapDispatchToProps)(LoiDan);
