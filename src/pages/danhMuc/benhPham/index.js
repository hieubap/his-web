...
const BenhPham = (props) => {
  ...
  useEffect(() => {
    ...
    searchBenhPham(params);
  }, []);

  useEffect(() => {
    const data = listBenhPham.map((item, index) => {
      return { ...item, action: item, stt: page * size + index + 1 };
    });
    setData(data);
  }, [listBenhPham, page, size]);
      ...
      <HomeWrapper title="Danh mục">
        ...
        >
          <TableWrapper
            ...
            buttonHeader={
              checkRole([ROLES["DANH_MUC"].BENH_PHAM_THEM])
                ? [
                    {
                      title: "Thêm mới [F1]",
        ...
        {!state.showFullTable && (
          ...
          >
            <CreatedWrapper
              ...
              onOk={handleAdded}
              okText="Lưu [F4]"
              roleSave={[ROLES["DANH_MUC"].BENH_PHAM_THEM]}
              roleEdit={[ROLES["DANH_MUC"].BENH_PHAM_SUA]}
              editStatus={editStatus}
            >
              <FormWraper
                disabled={
                  editStatus
                    ? !checkRole([ROLES["DANH_MUC"].BENH_PHAM_SUA])
                    : !checkRole([ROLES["DANH_MUC"].BENH_PHAM_THEM])
                }
                form={form}
                layout="vertical"
              ...
              >
                <Form.Item
                  ...
                  rules={[
                    {
                      required: true,
                      message: "Vui lòng nhập mã bệnh phẩm!",
                    },
                    {
                      max: 20,
                      message: "Vui lòng nhập mã bệnh phẩm không quá 20 ký tự!",
                    },
                    {
                      whitespace: true,
                      message: "Vui lòng nhập mã bệnh phẩm!",
                ...
                >
                  <Input
                    ...
                    placeholder="Vui lòng nhập mã bệnh phẩm"
                    ref={refAutoFocus}
                  />
                </Form.Item>
                <Form.Item
                  ...
                  rules={[
                    {
                      required: true,
                      message: "Vui lòng nhập tên bệnh phẩm!",
                    },
                    {
                      max: 1000,
                      message:
                        "Vui lòng nhập tên bệnh phẩm không quá 1000 ký tự!",
                    },
                    {
                      whitespace: true,
                      message: "Vui lòng nhập tên bệnh phẩm!",
                ...
                >
                  <Input
                    className="input-option"
                    placeholder="Vui lòng nhập tên bệnh phẩm"
                  />
                </Form.Item>
                {editStatus && (
...
const mapDispatchToProps = ({
  benhPham: { searchBenhPham, createOrEdit, onDelete, updateData },
}) => ({
  searchBenhPham,
  createOrEdit,
  onDelete,
...
export default connect(mapStateToProps, mapDispatchToProps)(BenhPham);
