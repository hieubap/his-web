...
const ViTriChanThuong = (props) => {
  ...
  useEffect(() => {
    ...
    getListViTriTranThuong(params);
  }, []);

  useEffect(() => {
    const data = listViTriTranThuong.map((item, index) => {
      return { ...item, action: item, stt: page * size + index + 1 };
    });
    setData(data);
  }, [listViTriTranThuong, page, size]);
      ...
      <HomeWrapper title="Danh mục">
        ...
        >
          <TableWrapper
            ...
            buttonHeader={
              checkRole([ROLES["DANH_MUC"].VI_TRI_CHAN_THUONG_THEM])
                ? [
                    {
                      title: "Thêm mới",
          ...
          >
            ...
            >
              <FormWraper
                disabled={
                  editStatus
                    ? !checkRole([ROLES["DANH_MUC"].VI_TRI_CHAN_THUONG_SUA])
                    : !checkRole([ROLES["DANH_MUC"].VI_TRI_CHAN_THUONG_THEM])
                }
                form={form}
                layout="vertical"
