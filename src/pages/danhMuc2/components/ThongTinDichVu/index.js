...
import { formatKetQuaThamChieu } from "components/DanhMuc/utils";
...
function FormServiceInfo(
  ...
  // trả data để truyền vào for.setfieldValues
  ...
  const customCreateOrEdit = (value, { resetField }) => {
    const {
      donViTinhId,
      giaBaoHiem,
      giaKhongBaoHiem,
    ...
    createOrEdit(bodySubmit, loaiDichVu).then(() => {
      if (!state.data?.id) {
        resetField();
      }
    });
  ...
  const renderForm = ({ form }) => {
    return (
      <>
        ...
        {[10, 20, 30, 40].includes(loaiDichVu) && (
